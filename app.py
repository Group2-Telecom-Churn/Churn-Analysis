...
# Streamlit Cloud runs the app from the repository root, but depending on the
# working directory is fragile; __file__ makes the app location-independent
# and tolerates the artifacts sitting either in models/ or beside app.py.
...
# The repo currently has BOTH a model/ and a models/ folder, so check each
# spelling before falling back to a file sitting next to app.py.
...
# Theming
#
# .streamlit/config.toml sets base = "dark", so every visitor lands in the
# dark palette regardless of their operating system preference. A viewer can
# still switch to Light from the app's Settings menu, so none of the colours
# below are hard-coded any more: they are chosen from the palette matching the
# theme the viewer is actually using. Streamlit exposes no CSS custom
# properties in the main app DOM, so this has to be resolved in Python via
# st.context.theme rather than in the stylesheet.
# --------------------------------------------------------------------------
...
DARK_PALETTE = {
    ...
    "meterTrack": "#26354f",
    "churnBackground": "rgba(239,68,68,.16)",
    "churnBorder": "rgba(239,68,68,.45)",
    "churnText": "#fecaca",
    "retainBackground": "rgba(34,197,94,.16)",
    "retainBorder": "rgba(34,197,94,.45)",
    "retainText": "#bbf7d0",
    "thresholdBackground": "rgba(59,130,246,.12)",
    "thresholdBorder": "rgba(59,130,246,.30)",
    "thresholdText": "#bfdbfe",
}
...
LIGHT_PALETTE = {
    ...
    "meterTrack": "#dbe2ec",
    "churnBackground": "rgba(239,68,68,.12)",
    "churnBorder": "rgba(239,68,68,.40)",
    "churnText": "#991b1b",
    "retainBackground": "rgba(34,197,94,.14)",
    "retainBorder": "rgba(22,163,74,.40)",
    "retainText": "#166534",
    "thresholdBackground": "rgba(59,130,246,.10)",
    "thresholdBorder": "rgba(59,130,246,.28)",
    "thresholdText": "#1e40af",
}
...
def active_palette():
    ...
    # unexpected falls back to dark, which is the configured default anyway.
    try:
...
STYLESHEET = Template(
    ...
    .hero h1 { margin-bottom: 4px; color: $heading; }
    .muted { color: $muted; }
    .card {
        padding: 18px;
        border-radius: 16px;
    ...
    .circle-value { font-size: 1.65rem; font-weight: 800; color: $heading; }
    .circle-label { font-size: .75rem; color: $muted; }
    .section-title {
        margin-top: 24px;
        margin-bottom: 10px;
        font-size: 1.15rem;
    ...
    .status-churn {
        padding: 15px;
        border-radius: 12px;
        background: $churnBackground;
        border: 1px solid $churnBorder;
        color: $churnText;
        font-weight: 700;
        text-align: center;
    }
    .status-retain {
        padding: 15px;
        border-radius: 12px;
        background: $retainBackground;
        border: 1px solid $retainBorder;
        color: $retainText;
        font-weight: 700;
        text-align: center;
    }
...
def seed_customer_registry():
    ...
    try:
        ...
        # Insert all 70 seed customers inside a single transaction rather than
        # opening, committing and closing a connection per row.
...
def transform_features(input_df, preprocessor):
    ...
    # which floods the deployment log. Re-attaching the column names produced
    # by the preprocessor keeps the prediction identical and silences it.
    transformed = preprocessor.transform(input_df)

    # Support preprocessors configured to emit a sparse matrix.
...
def get_customer_signals(input_df, preprocessor, model):
    ...
    for feature in MODEL_FEATURES:
        ...
        if feature == "tenure":
            ...
            alternatives = [
                "Yes" if original_value == "No" else "No"
            ]
        elif original_value in ["0", "1"]:
            alternatives = [
                "1" if original_value == "0" else "0"
            ]
        ...
        for alternative in alternatives:
            ...
            try:
                ...
                changes.append(
                    abs(base_probability - modified_probability)
                )
            except Exception:
                pass
    ...
    st.markdown(
        ...
        <div class="risk-card">
            <div style="
                ...
                background:conic-gradient({meter_color} {degrees}deg, {PALETTE['meterTrack']} 0deg);
                display:flex;
                align-items:center;
                justify-content:center;">
...
def generate_seed_scans(preprocessor, model):
    ...
    try:
        # Retrieve the historical customers loaded from the seed file.
        ...
        # Fetch every already-scanned ID in one query. The previous version
        # ran a separate SELECT per customer, on a fresh connection each time.
        already_scanned = {
            row["customerID"]
        ...
        # Score all pending customers in one vectorised call instead of
        # looping one predict_proba per customer.
        model_input = pd.concat(
            [load_customer_for_prediction(customer) for customer in pending],
            ignore_index=True,
        )
        transformed = transform_features(model_input, preprocessor)
...
@st.cache_resource(show_spinner="Preparing the churn model and registry...")
def bootstrap():
    # Streamlit re-executes this entire script on every widget interaction.
    # Without caching, each keystroke re-seeded the registry and re-checked
    # all 70 historical scans, opening and closing dozens of SQLite
    # connections per rerun. cache_resource runs this once per container.
    initialize_database()
...
st.markdown(
    """
    <div class="hero">
        <h1> Telecom Customer Churn APP</h1>
        <div class="muted">
...
if page == "Dashboard":
    ...
    # Stop before anything that needs the model. The customer search below
    # calls get_customer_signals(), which would raise on a None preprocessor
    # if this check stayed further down the page.
    ...
    if "latest_probability" in st.session_state:
        ...
        # Transposing the input frame mixes numbers and strings into a single
        # object column, which Arrow cannot serialise ("Could not convert
        # 'Male' with type str: tried to convert to double"). Casting to str
        # keeps the table renderable across pandas/pyarrow versions.
